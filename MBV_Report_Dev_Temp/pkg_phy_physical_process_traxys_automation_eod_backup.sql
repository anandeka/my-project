...
create or replace package body pkg_phy_physical_process is
  ...
  begin
    ...
    if vn_error_count = 0 then
      ...
      */
      ...
      if pc_process = 'EOM' then
        pkg_phy_mbv_report.sp_calc_derivative_diff_report(pc_corporate_id,
                                                          pd_trade_date,
                                                          pc_process_id,
                                                          pc_process,
                                                          pc_user_id);
      ...
      if pc_process = 'EOM' then
        pkg_phy_mbv_report.sp_calc_di_valuation_price(pc_corporate_id,
                               pd_trade_date,
                               pc_process,
                               pc_process_id,
                               pc_user_id);
      ...
      commit;
       ...
       if pc_process = 'EOM' then
        pkg_phy_mbv_report.sp_phy_postion_diff_report(pc_corporate_id,
                                                      pd_trade_date,
                                                      pc_process ,
                                                      pc_process_id);
      end if;
      commit;
      if pc_process = 'EOM' then
        pkg_phy_mbv_report.sp_allocation_report(pc_corporate_id ,
                                                pd_trade_date,
                                                pc_process,
                                                 pc_process_id);
      end if;
  ...
  begin
    ...
    delete from mbv_allocation_report_header where process_id = pc_process_id;
    delete from mbv_di_valuation_price where process_id = pc_process_id;
    delete from mbv_phy_postion_diff_report where process_id = pc_process_id;
    delete from mbv_derivative_diff_report where process_id = pc_process_id;    
    commit;
    --end Suresh 
    sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
