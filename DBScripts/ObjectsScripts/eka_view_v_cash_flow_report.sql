...
SELECT iss.corporate_id, akc.groupid,
       ...
       NVL (cpc.profit_center_name,
            cpc1.profit_center_name
           ) profit_center_name,
       NVL (cpc.business_line_id, cpc1.business_line_id) business_line_id,
       css.strategy_id, css.strategy_name,
       NVL (pcm.partnership_type, 'Normal') execution_type, pdm.product_id,
       pdm.product_desc product_name, pdm.product_type_id,
       pdm.product_group_id, pgm.product_group_name,
       (CASE
           WHEN NVL (pcm.purchase_sales, 'NA') = 'P'
              THEN 'Purchase'
           WHEN NVL (pcm.purchase_sales, 'NA') = 'S'
              THEN 'Sales'
           ELSE 'NA'
        END
       ) contract_type,
       iss.internal_invoice_ref_no, iss.internal_contract_ref_no,
       iss.invoice_ref_no,
       (CASE
           WHEN iss.invoice_type_name = 'AdvancePayment'
              THEN 'Commercial'
           ELSE iss.invoice_type
        END
       ) invoice_type,
       iss.invoice_type_name, phd.profileid cp_id, phd.companyname cp_name,
       pad.city_id, cim.city_name, pad.country_id, cym.country_name,
         ROUND (iss.total_invoice_item_amount, 4)
       * (CASE
             WHEN NVL (iss.recieved_raised_type, 'NA') = 'Raised'
                THEN 1
             WHEN NVL (iss.recieved_raised_type, 'NA') = 'Received'
                THEN -1
             ELSE (CASE
       ...
       (CASE
           WHEN iss.payment_due_date IS NOT NULL
              THEN TO_CHAR (iss.payment_due_date, 'dd/mm/yyyy')
           ELSE ''
        END
       ) payment_due_date,
       (CASE
           WHEN iss.invoice_issue_date IS NOT NULL
              THEN TO_CHAR (iss.invoice_issue_date, 'dd/mm/yyyy')
           ELSE ''
        END
       ) invoice_issue_date,
       iss.cp_ref_no, pym.payment_term credit_term,
       ...
       * (CASE
          ...
          END
         ) invoice_amount_in_payin_cur,
       iss.invoice_cur_id invoice_pay_in_cur_id,
       (CASE
           WHEN NVL (iss.invoice_cur_id, 'NA') = akc.base_cur_id
              THEN 1
           ELSE NVL (iss.fx_to_base, 1)
        END
       ) fx_to_base,
         ROUND (iss.total_amount_to_pay, 4)
       * (CASE
  ...
  FROM is_invoice_summary iss,
       ...
       (SELECT pad.address_id, pad.address_type, pad.profile_id, pad.city_id,
               pad.country_id, pad.address_name
 ...
 WHERE iss.is_active = 'Y'
   ...
   AND 'TRUE' =
          (CASE
              WHEN iss.invoice_type_name = 'AdvancePayment'
                 THEN 'TRUE'
              WHEN iss.invoice_type_name = 'Profoma'
                 THEN 'FALSE'
              ELSE (CASE
                       WHEN NVL (iss.invoice_type, 'NA') = 'Commercial'
                          THEN 'TRUE'
