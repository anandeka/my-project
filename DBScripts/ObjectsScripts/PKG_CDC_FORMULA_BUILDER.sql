...
CREATE OR REPLACE PACKAGE BODY "PKG_CDC_FORMULA_BUILDER" is
  ...
  procedure sp_calculate_forumla(pobj_fb_setup                fb_tbl_setup,
                                 ...
                                 pobj_out_fb_instrument_data  out fb_tbl_instrument_data,
                                 pobj_out_fb_price_by_formula out tp_tbl_formula,
                                 pobj_out_fb_tbl_error        out fb_tbl_error) as
  ...
  begin
    ...
    select count(*)
      into vn_setup_count
      from (select price_unit_id, corporate_id
    ...
    if vn_setup_count = 0 then
      ...
      for cur_setup_rows in cur_setup loop
        ...
        select count(*)
          ...
          from (select fb_order_seq, instrument_id
                  from the (select cast(pobj_fb_instrument_data as
                                        fb_tbl_instrument_data)
                              from dual)) tt,
               dim_der_instrument_master dim,
        ...
        if vn_instrument_count = 0 then
          ...
          for cur_instrument_rows in cur_instrument loop
            ...
            if cur_setup_rows.formula_id = cur_instrument_rows.formula_id then
                ...
                if cur_instrument_rows.fb_period_type = 'Prompt' and
                  ...
                  begin
                    select nvl(drm.last_tradable_date, drm.prompt_date) -- nvl added to avoid null error in metal exhcnage
                      into vd_start_date
                      from drm_derivative_master drm, pm_period_master pm
                     where drm.instrument_id =
                           cur_instrument_rows.instrument_id
                       and drm.period_month =
                           cur_instrument_rows.period_month
              ...
              if vc_inst_price_status_flag <> 'Error' then
                ...
                vn_row := 1;
                while vd_last_wkg_date >= vd_first_wkg_date loop
                ...
                if cur_instrument_rows.price_point_id is not null then
                  ...
                  if vc_calculated_dr_id is not null then
                    ...
                    for i in vc_tbl.first .. vc_tbl.last loop
                  ...
                  if cur_instrument_rows.fb_period_type in
                    ...
                    begin
                      select drm.dr_id,
                        ...
                        into vc_calculated_dr_id, vd_drm_last_tradable_date
                        from drm_derivative_master      drm,
                             pm_period_master           pm,
                             dim_der_instrument_master  dim,
                             irm_instrument_type_master irm
                  ...
                  if vc_calculated_dr_id is null then
                    ...
                    for i in vc_tbl.first .. vc_tbl.last loop
                      if i = 1 then
                        begin
                          select t.dr_id, t.last_tradable_date
                            into vc_calculated_dr_id,
                                 vd_drm_last_tradable_date
                ...
                for i in vc_tbl.first .. vc_tbl.last loop
                  ...
                  if cur_instrument_rows.is_spot_instrument = 'Y' and
                    ...
                    begin
                      select price, price_unit_id, trade_date
                        into vc_tbl(i) .price,
                             vc_tbl(i) .price_unit_id,
                             vc_tbl(i) .vd_avl_price_date
                ...
                loop
                  if vc_tbl(i).tradedate is not null then
                    dbms_output.put_line(vc_tbl(i)
                                         .tradedate || ' ,' || vc_tbl(i)
                                         .instrumentid || ' ,' || vc_tbl(i).isholiday || ' , ' || vc_tbl(i)
                                         .drid || ' ,' || vc_tbl(i)
                                         .price || ', price date := ' ||
                                         vc_tbl(i).vd_avl_price_date);
                  end if;
                end loop;*/
                --while loop removed by siva on 30Mar2011, as it's not required to repeat
                for i in vc_tbl.first .. vc_tbl.last loop
              ...
              /*DISPLAYING THE TYPE*/
              /*dbms_output.put_line('test output data');
              for i in vc_tbl_out.first .. vc_tbl_out.last
              loop
                  dbms_output.put_line(vc_tbl_out(i).tradedate || ' ,' ||
                                       vc_tbl_out(i).instrumentid || ' ,' ||
                                       vc_tbl_out(i).isholiday || ' , ' ||
                                       vc_tbl_out(i).drid || ' ,' ||
                                       vc_tbl_out(i).price ||
                                       ', price date := ' || vc_tbl_out(i).vd_avl_price_date);
    ...
    if vc_inst_tbl.exists(1) and vobj_fb_tbl_setup.exists(1) then
      for i in vc_inst_tbl.first .. vc_inst_tbl.last loop
      ...
      begin
        for i in vc_inst_tbl(1).v_tbl_formula.first .. vc_inst_tbl(1)
                                                      .v_tbl_formula.last loop
          vc_tbl_daywise.extend;
          vc_formula_exe := vc_formula;
          for vn_inst_total in 1 .. vc_inst_tbl.last loop
          ...
          /*          dbms_output.put_line('The Day    ' || i ||
                                         '  formula operation is :- ' ||
                                         vc_formula_exe || ' and its  value is :- ' ||
                                         vn_daywise_price);
          -- setting the day wise price into the formula_daywise type   */
        
          if vc_inst_tbl(1).v_tbl_formula(i)
          .vd_avl_price_date = vc_inst_tbl(1).v_tbl_formula(i).tradedate then
            vc_final_price_status_flag := 'Final';
          else
            vc_final_price_status_flag := 'Provisional';
          end if;
          if vc_inst_tbl(1).v_tbl_formula(i).isholiday = 'N' then
            if vc_setup_price_status_flag <> 'Provisional' then
              vc_setup_price_status_flag := vc_final_price_status_flag;
            end if;
          end if;
          /* dbms_output.put_line('Trade Date   '||vc_inst_tbl(1) .v_tbl_formula(i).tradedate        
           ||'  Available Price date   '||vc_inst_tbl(1) .v_tbl_formula(i).vd_avl_price_date
          || '  Price is  '||vn_daywise_price
          ||'Is holiday  '||vc_inst_tbl(1) .v_tbl_formula(i).isholiday ||  'vc_final_price_status_flag'|| vc_final_price_status_flag);*/
          vc_tbl_daywise(i) := tp_obj_formula(vc_inst_tbl(1)
                                              .v_tbl_formula(i).tradedate,
                                              null,
                                              vc_inst_tbl(1)
                                              .v_tbl_formula(i).isholiday,
                                              null,
      ...
      /* --to update the final fb_price_status and fb_price_qp_status
      ...
      if vc_final_price_status_flag <> 'Error' then
        begin
          --vc_sql_temp := 'select  ( ' || vc_formula || ' )  from dual';
          --        dbms_output.put_line('vc_sql_temp ' || vc_sql_temp);
          -- execute immediate vc_sql_temp   into vn_average;
          vc_sql_temp := vc_formula;
          vn_average  := fn_execute_internal_formula(vc_sql_temp);
          /*dbms_output.put_line(' The  formula is  -- ' || vc_sql_temp ||
          ' aand  the value for the instrumet in the month wise is ---  ' ||
          vn_average);*/
      ...
      vobj_fb_tbl_setup(1).fb_price_staus := vc_setup_price_status_flag;
  ...
  begin
    ...
    for cur_err_log_rows in cur_err_log loop
  ...
  begin
    ...
    for cur_fbs_log_rows in cur_fbs_log loop
    ...
    for cur_fbi_log_rows in cur_fbi_log loop
  ...
  begin
    for cc in cur_fbs_formula loop
  ...
  begin
    ...
    begin
      select scd.cur_id, scd.factor
        into vc_from_main_cur_id, vn_from_main_currency_factor
        from cm_currency_master cm, scd_sub_currency_detail scd
    ...
    begin
      select scd.cur_id, scd.factor
        into vc_to_main_cur_id, vn_to_main_currency_factor
        from cm_currency_master cm, scd_sub_currency_detail scd
       where cm.cur_id = scd.cur_id
