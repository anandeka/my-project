-- Procedure to populate the Bulk pricing document table.
...
-- Bulk Price fixation -- Header details
   INSERT INTO bpfd_bulk_pfd_d
               (internal_doc_ref_no, doc_issue_date, corporate_name,
                cp_address, cp_city, cp_country, cp_zip, cp_state, cp_name,
                cp_person_in_charge, contract_type, purchase_sales,
                contract_ref_no, delivery_item_ref_no, pay_in_currency,
                product, quality, gmr_ref_no, quota_period)
      SELECT DISTINCT p_docrefno, p_doc_issue_date, ak.corporate_name,
                      pad.address, cim.city_name, cym.country_name, pad.zip,
                      sm.state_name, phd.companyname companyname,
                      (gab.firstname || ' ' || gab.lastname) personincharge,
                      pcm.contract_type,
                      (CASE
...
-- three different select queries -- Event Based, DI based (non PriceAllocation ) , DI Based Price Allocation (Non Pledge GMR)
-- TODO: DI Based Price Allocation - Pledge GMR ??
...
--Delivery Item based
      SELECT p_docrefno, pfd.pfd_id, aml.attribute_name elementname,
       ...
       WHERE gmr.internal_gmr_ref_no = gcim.internal_gmr_ref_no
         ...
         AND pcdi.price_allocation_method NOT IN ('Price Allocation')
         AND gmr.internal_gmr_ref_no = p_internal_gmr_ref_no
-- price allocation non pledged gmr
      UNION ALL
      SELECT p_docrefno, pfd.pfd_id, aml.attribute_name elementname,
             axs.action_ref_no pricefixationrefno,
             TO_CHAR (pfd.as_of_date, 'dd-Mon-YYYY') pricefixationdate,
