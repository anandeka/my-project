...
-- 1) Open Contracts
   select 'Standard' product_type,
         ...
         when pcm.purchase_sales = 'P' and pcm.is_tolling_contract = 'Y' and
          ...
          'Open '|| decode(pcm.purchase_sales, 'P', 'Purchase', 'Sales') position_sub_type,
          ...
          CASE
             ...
             ELSE 'NA'
          END dest_city_name,
          CASE WHEN itm.location_field = 'DESTINATION' THEN sm_pcdb.state_id ELSE 'NA' END dest_state_id, -- Newly Added
          CASE WHEN itm.location_field = 'DESTINATION' THEN sm_pcdb.state_name ELSE 'NA' END dest_state_name, -- Newly Added
          CASE WHEN itm.location_field = 'DESTINATION' THEN rem_pcdb.region_name ELSE 'NA' END  dest_loc_group_name, -- Newly Added
          pci.expected_delivery_month || '-'|| pci.expected_delivery_year period_month_year,
          CASE
             WHEN pci.delivery_period_type = 'Date' AND pci.is_called_off = 'Y'
...
--          ciqs.open_qty * ucm_base.multiplication_factor qty_in_base_unit,
          ciqs.open_qty * pkg_general.f_get_converted_quantity(pcpd.product_id,
  ...
  SELECT 'Standard' product_type,
         ...
         when pcm.purchase_sales = 'P' and pcm.is_tolling_contract = 'Y' and
          ...
          gab.firstname || ' ' || gab.lastname trader_name,
           pcm.partnership_type Execution_Type,
          'NA' broker_profile_id, 'NA' broker_name,
          pci.incoterm_id incoterm_id, itm.incoterm incoterm,
          pci.payment_term_id payment_term_id, pym.payment_term payment_term,
  ...
  SELECT 'Standard' product_type,
         ...
         'Physical' position_type,
          ...
          gab.firstname || ' ' || gab.lastname trader_name,
           pcm.partnership_type Execution_Type,
          'NA' broker_profile_id, 'NA' broker_name,
          itm.incoterm_id incoterm_id, itm.incoterm incoterm,
          pym.payment_term_id payment_term_id, pym.payment_term payment_term,
          'NA' origination_country_id, 'NA' origination_country,
          'NA' origination_city_id, 'NA' origination_city,
          'NA' price_type_name, cm_invoice_curreny.cur_id pay_in_cur_id,
    ...
    WHERE dgrd.internal_gmr_ref_no = gmr.internal_gmr_ref_no
      ...
      and pcm.internal_contract_ref_no = gmr.internal_contract_ref_no -- Newly Added
      and pcm.internal_contract_ref_no = pcmte.int_contract_ref_no(+) -- Newly Added
      AND pcm.internal_contract_ref_no = pcpd.internal_contract_ref_no -- Newly Added
      and pdm.product_group_id = pgm.product_group_id -- Newly Added
      AND pdm.product_type_id = pdtm.product_type_id
      AND pdtm.product_type_name = 'Standard'
      AND pdm.base_quantity_unit = qum.qty_unit_id
   ...
   SELECT 'Standard' product_type, 'Base Metal Stocks' section_name,
         ...
         when pcm.purchase_sales = 'P' and pcm.is_tolling_contract = 'Y' and
          ...
          'NA' counter_party_name, 'NA', 'NA' trader_name,
           pcm.partnership_type Execution_Type,
          'NA' broker_profile_id, 'NA' broker_name, 'NA' incoterm_id,
    ...
    WHERE grd.internal_gmr_ref_no = gmr.internal_gmr_ref_no
     ...
     -- AND v_qat.quality_id = qat.quality_id
      ...
      AND cm_invoice_currency.cur_id(+) = pci.invoice_currency_id
      AND cym_gmr_dest_country.country_id(+) = gmr.discharge_country_id -- Modified
      AND cim_gmr_dest_city.city_id(+) = gmr.discharge_city_id -- Modified
      and cim_gmr_dest_city.state_id = sm_gmr_dest_state.state_id(+) -- Newly Added
      and cym_gmr_dest_country.region_id = rem_dest.region_id(+) -- Newly Added
      AND qum_gcd.qty_unit_id = gcd.group_qty_unit_id
      AND grd.qty_unit_id = ucm.from_qty_unit_id
      AND gcd.group_qty_unit_id = ucm.to_qty_unit_id
      AND cm_base_currency.cur_id = akc.base_cur_id
