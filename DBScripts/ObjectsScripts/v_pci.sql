...
SELECT pci.internal_contract_item_ref_no AS internal_contract_item_ref_no,
       ...
       ciqs.gmr_qty AS gmr_qty,
       ciqs.shipped_qty AS shipped_qty,
       0 AS warehouse_qty,
       ...
       (CASE
           WHEN ciqs.allocated_qty > 0 THEN
            'Allocated'
           ELSE
            'Un-allocated'
       ...
       (CASE
           WHEN pci.delivery_period_type = 'Month' THEN
            CASE
           WHEN pci.delivery_from_month = pci.delivery_to_month AND
                pci.delivery_from_year = pci.delivery_to_year THEN
            pci.delivery_from_month || ' ' || pci.delivery_from_year
           ELSE
            pci.delivery_from_month || ' ' || pci.delivery_from_year || ' To ' ||
            pci.delivery_to_month || ' ' || pci.delivery_to_year
       END WHEN pci.delivery_period_type = 'Date' THEN CASE
            WHEN to_char(pci.delivery_from_date,
                         'dd-Mon-YYYY') =
                 to_char(pci.delivery_to_date,
                         'dd-Mon-YYYY') THEN
             to_char(pci.delivery_from_date,
                     'dd-Mon-YYYY')
            ELSE
             to_char(pci.delivery_from_date,
                     'dd-Mon-YYYY') || ' To ' ||
             to_char(pci.delivery_to_date,
                     'dd-Mon-YYYY')
        END ELSE '-' END) quota_month,
       (nvl(cim.city_name,
            '') || cym.country_name) AS location,
       (itm.incoterm || ', ' || cim.city_name || ', ' || cym.country_name) AS incoterm_location,
       ...
       nvl(pcdi.tolerance_type,
           'Approx') tolerance_type,
       nvl(pcdi.min_tolerance,
           0) tolerance_min,
       nvl(pcdi.max_tolerance,
           0) tolerance_max,
       pcdi.tolerance_unit_id,
       (CASE
           WHEN pcdi.tolerance_type = 'Percentage' THEN
            pci.item_qty - pci.item_qty * (pcdi.min_tolerance / 100)
           ELSE
            pci.item_qty
       END) min_tolerance_item_qty,
       (CASE
           WHEN pcdi.tolerance_type = 'Percentage' THEN
            pci.item_qty + pci.item_qty * (pcdi.max_tolerance / 100)
           ELSE
            pci.item_qty
       END) max_tolerance_item_qty,
       css.strategy_name,
       (gab.firstname || ' ' || gab.lastname) AS trader,
