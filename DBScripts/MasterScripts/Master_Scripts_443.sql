...
SELECT '''',
       ...
       '''',
       (SELECT f_string_aggregate(distinct AGRD.CONTAINER_NO)
        FROM   agrd_action_grd agrd
        WHERE  agrd.internal_gmr_ref_no = agmr.internal_gmr_ref_no
        AND    agrd.action_no = agmr.action_no
...
SELECT '''''''',
       ...
       vd.carriers_agents_endorsements carrieragentsendorsements,
       vd.voyage_ref_no,
       (SELECT f_string_aggregate(distinct dgrd.container_no)
...
begin
 ...
 update DGM_DOCUMENT_GENERATION_MASTER  set FETCH_QUERY=fetchqry where DGM_ID IN ('1','2','3','4');
