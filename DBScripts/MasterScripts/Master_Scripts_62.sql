...
Insert into DGM_DOCUMENT_GENERATION_MASTER
 ...
 Values
   ('18', 'weightNote', 'Purchase Weight Note', 'weightNote', 1, 
    'NULL', 'N');

NOTE: HAVE to REPLACE NULL WITH THE Insert Query of Weightnote
...
/***************************************************************************************/
This is for shipmentAdvise,airAdvice,railAdvice and truckAdvice
NOTE:- This below insert query need to put manually for DGM_ID 1,2,3,4 in FETCH_QUERY column.
/***************************************************************************************/
INSERT INTO SAD_D(
ATTENTION,
BUYER,
...
SELECT '''',
       ...
       gab.firstname || ',' || gab.lastname notifyparty,
       phd_shipper.companyname shipper,
       vd.notes notes,
       vd.special_instructions specialinst,
       vd.voyage_number voyagenumber,
       vd.shippers_ref_no shipperrefno,
       ...
       (SELECT f_string_aggregate(dgrd.container_no)
         ...
         WHERE dgrd.internal_gmr_ref_no = agmr.internal_gmr_ref_no) container_no,
       ...
       qumbl.qty_unit net_weight_unit_gmr,
       '',
       (TO_CHAR(sad.bl_date, 'dd-Mon-yyyy') || ' ' || sad.bl_no) bldate_blno,
       '''',
       TO_CHAR(axs.action_date, 'dd-Mon-yyyy') activity_date,
       vd.voyage_number flight_number,
       cym_vd.country_name destination_airport,
       TO_CHAR(sad.bl_date, 'dd-Mon-yyyy') awb_date,
       sad.bl_no awb_number,
       f_format_to_char(gmr.qty,4) awb_quantity,
       '''',
       vd.loading_date loading_date,
       vd.carriers_agents_endorsements,
       '',
       vd.no_of_pieces no_of_pieces,
       vd.nature_of_goods nature_of_good,
       vd.dimensions dimensions,
       (SELECT f_string_aggregate(dgrd.internal_stock_ref_no)
        ...
        f_format_to_char((SELECT SUM(pkg_general.f_get_converted_quantity(adgrd.product_id,
         ...
         WHERE adgrd.internal_gmr_ref_no = agmr.internal_gmr_ref_no
           ...
           AND adgrd.action_no = agmr.action_no),4) net_weight,
       f_format_to_char((SELECT SUM(pkg_general.f_get_converted_quantity(adgrd.product_id,
                                                        adgrd.net_weight_unit_id,
                                                        qum.qty_unit_id,
                                                        adgrd.tare_weight))
         ...
         WHERE adgrd.internal_gmr_ref_no = agmr.internal_gmr_ref_no
           AND adgrd.status = 'Active'
           AND adgrd.action_no = agmr.action_no),4) tare_weight,
       f_format_to_char((SELECT SUM(pkg_general.f_get_converted_quantity(adgrd.product_id,
                                                        adgrd.net_weight_unit_id,
                                                        qum.qty_unit_id,
                                                        adgrd.gross_weight))
         ...
         WHERE adgrd.internal_gmr_ref_no = agmr.internal_gmr_ref_no
           AND adgrd.status = 'Active'
           AND adgrd.action_no = agmr.action_no),4)gross_weight,
       vd.handling_instructions,
       sad.internal_remarks comments,
       '',
       '',
       '',
       '',
       '',
       (vd.vessel_voyage_name || ' ' || vd.voyage_number) rail_name_number,
       TO_CHAR(sad.bl_date, 'dd-Mon-yyyy') rr_date,
       sad.bl_no rr_number,
       f_format_to_char(gmr.qty,4) total_qty,
       f_format_to_char(gmr.qty,4) rr_qty,
       vd.shippers_ref_no truck_number,
       sad.bl_date cmr_date,
       sad.bl_no cmr_number,
 ...
 WHERE gmr.internal_gmr_ref_no = sad.internal_gmr_ref_no
   ...
   AND sad.notify_party_id = gab.gabid(+)
   AND gab.profileid = phdnp.profileid(+)
   AND vd.discharge_port_id = pmt_vd.port_id(+)
   AND vd.discharge_country_id = cym_vd.country_id(+)
