...
BEGIN
   UPDATE dgm_document_generation_master dgm
      SET dgm.fetch_query = fetchquerycpayd
    WHERE dgm.dgm_id IN
                ('CREATE_DFT_PIC_2', 'CREATE_DFT_FIC_2', 'CREATE_DFT_DFIC_2')
      AND dgm.doc_id IN ('CREATE_DFT_PI', 'CREATE_DFT_FI', 'CREATE_DFT_DFI')
      AND dgm.sequence_order IN (2)
      AND dgm.is_concentrate = 'Y';

   UPDATE dgm_document_generation_master dgm
      SET dgm.fetch_query = fetchqueryctcd
    WHERE dgm.dgm_id IN
                ('CREATE_DFT_PIC_3', 'CREATE_DFT_FIC_3', 'CREATE_DFT_DFIC_3')
      AND dgm.doc_id IN ('CREATE_DFT_PI', 'CREATE_DFT_FI', 'CREATE_DFT_DFI')
      AND dgm.sequence_order IN (3)
      AND dgm.is_concentrate = 'Y';
