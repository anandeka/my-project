declare fetchQueryISDAPIDraft clob :=
'INSERT INTO is_d
...
fetchQueryAPIDDraft clob :='INSERT INTO api_d
            (internal_invoice_ref_no, contract_ref_no, product,
             invoice_quantity, invoice_quantity_unit, quality,
             invoice_amount_unit, invoice_amount, price, price_unit,
             invoice_item_amount, internal_doc_ref_no)
   SELECT   invs.internal_invoice_ref_no AS internal_invoice_ref_no,
            pcm.contract_ref_no AS contract_ref_no,
            pdm.product_desc AS product,
            apid.invoice_item_qty AS invoice_quantity,
            qum.qty_unit AS invoice_quantity_unit,
            qat.quality_name AS quality, cm.cur_code AS invoice_amount_unit,
            invs.total_amount_to_pay AS invoice_amount,
            apid.invoice_item_price AS price,
            pum.price_unit_name AS price_unit,
            invs.total_invoice_item_amount AS invoice_item_amount, ?
       FROM is_invoice_summary invs,
            apid_adv_payment_item_details apid,
            pcm_physical_contract_main pcm,
            v_pci pci,
   ...
   GROUP BY pcm.contract_ref_no,
            ...
            invs.internal_invoice_ref_no,
            qum.qty_unit,
            invs.total_invoice_item_amount,
            apid.invoice_item_qty';           
begin

UPDATE DGM_DOCUMENT_GENERATION_MASTER DGM SET DGM.FETCH_QUERY = fetchQueryISDAPIDraft WHERE DGM.DGM_ID IN ('CREATE_DFT_API_1','CREATE_DFT_APIC_1') AND DGM.DOC_ID IN ('CREATE_DFT_API') AND DGM.SEQUENCE_ORDER=1;

UPDATE DGM_DOCUMENT_GENERATION_MASTER DGM SET DGM.FETCH_QUERY = fetchQueryAPIDDraft WHERE DGM.DGM_ID IN ('CREATE_DFT_API_2','CREATE_DFT_APIC_2') AND DGM.DOC_ID IN ('CREATE_DFT_API') AND DGM.SEQUENCE_ORDER=2;
commit;
end;