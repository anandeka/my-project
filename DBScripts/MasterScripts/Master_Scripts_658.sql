DECLARE fetchQueryDGMIGD CLOB := 'INSERT INTO igd_inv_gmr_details_d
...
fetchQueryDGMIGDB CLOB := 'INSERT INTO igd_inv_gmr_details_d
            (internal_invoice_ref_no, gmr_ref_no, container_name,
             mode_of_transport, bl_date, origin_city, origin_country, wet_qty,
   ...
   SELECT   invs.internal_invoice_ref_no AS internal_invoice_ref_no,
            ...
            stragg (DISTINCT (NVL (cim.city_name, ''''))) AS origin_city,
            stragg (DISTINCT (NVL (cym.country_name, ''''))) AS origin_country,
            gmr.qty AS wet_qty, qum.qty_unit AS wet_qty_unit_name, ?
       FROM is_invoice_summary invs,
            iid_invoicable_item_details iid,
            grd_goods_record_detail grd,
            dgrd_delivered_grd dgrd,
...
BEGIN
UPDATE DGM_DOCUMENT_GENERATION_MASTER DGM SET DGM.FETCH_QUERY=fetchQueryDGMIGD WHERE DGM.DGM_ID IN ('CREATE_DFT_FIC_12','CREATE_DFT_PIC_12','CREATE_DFT_DFIC_12') AND DGM.DOC_ID IN ('CREATE_DFT_FI','CREATE_DFT_PI','CREATE_DFT_DFI') AND DGM.SEQUENCE_ORDER IN (12) AND DGM.IS_CONCENTRATE = 'Y';

UPDATE DGM_DOCUMENT_GENERATION_MASTER DGM SET DGM.FETCH_QUERY=fetchQueryDGMIGDB WHERE DGM.DGM_ID IN ('DGM-DFT-DFI-C11','DGM-DFT-FI-C11','DGM-DFT-PI-C9') AND DGM.DOC_ID IN ('CREATE_DFT_DFI','CREATE_DFT_FI','CREATE_DFT_PI') AND DGM.SEQUENCE_ORDER IN (9,11) AND DGM.IS_CONCENTRATE = 'N';
