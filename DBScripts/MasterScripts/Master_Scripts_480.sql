...
LOADING_STATE,
DESTINATION_STATE,
TRANS_SHIPMENT_COUNTRY,
TRANS_SHIPMENT_STATE
)
SELECT '''', akc.corporate_name buyer, pcm.issue_date contractdate,
       ...
       f_format_to_char (gmr.qty, 4) quantity, qum_gmr.qty_unit quantity_unit,
       ...
       f_format_to_char (gmr.qty, 4) rr_qty, vd.shippers_ref_no truck_number,
       sd.bl_date cmr_date, sd.bl_no cmr_number,
...
SELECT '''''''', pci.cp_name buyer, pci.issue_date contractdate,
       ...
       gmr.created_date createddate, pci.incoterm_location parity_location,
       ...
       (SELECT f_string_aggregate (dgrd.container_no)
         ...
         WHERE dgrd.internal_gmr_ref_no =
                                        agmr.internal_gmr_ref_no)
                                                                 container_no,
       f_format_to_char (gmr.qty, 4) quantity, qumbl.qty_unit quantity_unit,
       ...
       '''', '''',
       (vd.vessel_voyage_name || '' '' || vd.voyage_number) rail_name_number,
       TO_CHAR (sad.bl_date, ''dd-Mon-yyyy'') rr_date, sad.bl_no rr_number,
       f_format_to_char (gmr.qty, 4) total_qty,
       f_format_to_char (gmr.qty, 4) rr_qty, vd.shippers_ref_no truck_number,
       sad.bl_date cmr_date, sad.bl_no cmr_number,
       f_format_to_char (gmr.qty, 4) cmr_quantity, '''',
