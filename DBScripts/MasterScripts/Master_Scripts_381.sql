...
(CASE
   WHEN INVS.IS_FREE_METAL = ''Y'' OR INVS.IS_PLEDGE = ''Y''
      THEN ''-''
   ELSE stragg(distinct QUM_GMR.QTY_UNIT)
END
) AS INVOICED_QTY_UNIT,
...
nvl(stragg(PHD1.COMPANYNAME), stragg(PHD2.COMPANYNAME)) as NOTIFY_PARTY, 
...
BEGIN
 update DGM_DOCUMENT_GENERATION_MASTER dgm set DGM.FETCH_QUERY = fetchQry1 where DOC_ID IN ('CREATE_PI','CREATE_DFI','CREATE_FI') AND IS_CONCENTRATE = 'N' AND SEQUENCE_ORDER = 1;
 commit;
END;