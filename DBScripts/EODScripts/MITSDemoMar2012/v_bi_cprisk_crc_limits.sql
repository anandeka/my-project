...
select t.profile_id,
      ...
      -- t.contract_type,
       ...
       sum((case when t.contract_type ='S' then -1 else 1 end)*t.net_exposure_limit) net_exposure_limit,
  ...
  from (select crc.limit_id profile_id,
        ...
        select crc.limit_id profile_id,
         ...
         group by crc.limit_id,
                  cpc.corporateid, crc.contract_type,
                  akc.corporate_name,
                  crc.exposure_qty_unit_id,
                  crc.exposure_curr_id) t
 group by t.profile_id,
          t.corporate_id,
          t.corporate_name,--t.contract_type,
          t.org_level_id,
          t.exposure_qty_unit_id,
          t.exposure_curr_id 
