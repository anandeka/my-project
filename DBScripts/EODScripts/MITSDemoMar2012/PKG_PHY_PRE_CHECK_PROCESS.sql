create or replace package "PKG_PHY_PRE_CHECK_PROCESS" is
  ...
  function fn_get_val_drid(pc_corporate_id       in varchar2,
                           ...
                           pd_qp_end_date        date) return varchar2;
...
create or replace package body "PKG_PHY_PRE_CHECK_PROCESS" is
  ...
  procedure sp_pre_check_m2m_values(pc_corporate_id varchar2,
    ...
    ******************************************************************************************************************************************/
    ...
    vd_valuation_date           date;
    vn_days_forward             number;
  begin
    ...
    insert into tmpc_temp_m2m_pre_check
      ...
      (select pcm.corporate_id corporate_id,
              ...
              nvl(pci.qp_end_date, pd_trade_date) qp_end_date,
              pci.price_basis
    ...
    for cur_pofh in (select pofh.internal_gmr_ref_no,
                            pofh.qp_end_date
                       from pofh_price_opt_fixation_header pofh,
                            tmpc_temp_m2m_pre_check        tmpc
                      where pofh.internal_gmr_ref_no =
                            tmpc.internal_gmr_ref_no
                        and tmpc.corporate_id = pc_corporate_id)
    loop
      update tmpc_temp_m2m_pre_check tmpc
      ...
      if cc.eval_basis <> 'FIXED' then
        ...
        update tmpc_temp_m2m_pre_check tmpc
         ...
         where tmpc.instrument_id = cc.instrument_id
           ...
           and tmpc.qp_end_date = cc.qp_end_date;
      end if;
    end loop;
    commit;
  ...
  procedure sp_pre_check_m2m_conc_values(pc_corporate_id varchar2,
    ...
    ******************************************************************************************************************************************/
    ...
    vn_qty_premimum_amt         number;
    vn_pp_amt                   number;
    vc_quality_exch_rate_string varchar2(100);
    vc_exch_rate_missing        varchar2(1);
    vc_product_exch_rate_string varchar2(100);
  ...
  function fn_get_val_drid(pc_corporate_id       in varchar2,
                           ...
                           p_trade_type          in varchar2,
                           p_process             in varchar2,
                           pd_qp_end_date        date) return varchar2 is
    v_date                  date;
    v_trade_date            date;
    vobj_error_log          tableofpelerrorlog := tableofpelerrorlog();
    vn_eel_error_count      number := 1;
  ...
  begin
    ...
    if p_trade_type = 'STOCK' then
      -- it is based on QP End Date for Stock
      if is_daily_app = 'Y' then
        v_date := pd_qp_end_date;
        -- Move it by 2 days
        vn_days_forward := 0;
        v_date          := v_date + 1;
        while vn_days_forward <> 2
        loop
          if pkg_cdc_derivatives_process.f_is_day_holiday(p_instrument_id,
                                                          v_date) then
            v_date := v_date + 1;
          else
            vn_days_forward := vn_days_forward + 1;
            if vn_days_forward <> 2 then
              v_date := v_date + 1;
            end if;
          end if;
        end loop;
        ...
        vd_value_date := v_date; -- Required for error display
      elsif is_daily_app = 'N' and is_month_app = 'Y' then
        -- Move to next Month
        if to_date('01-' || to_char(pd_qp_end_date, 'Mon') || '-' ||
                   to_char(pd_qp_end_date, 'YYYY'),
                   'dd-Mon-yyyy') <= p_trade_date then
          vd_value_date := pkg_metals_general.fn_get_next_month_prompt_date(vc_delivery_calendar_id,
                                                                            pd_qp_end_date);
        
        else
          vd_value_date := to_date('01-' || to_char(pd_qp_end_date, 'Mon') || '-' ||
                                   to_char(pd_qp_end_date, 'YYYY'),
                                   'dd-Mon-yyyy');
        end if;
        begin
      ...
      if is_daily_app = 'Y' then
        if p_trade_type = 'OPEN' then
          v_date := to_date('01' || p_from_month || p_from_year,
                            'dd-mm-yyyy');
