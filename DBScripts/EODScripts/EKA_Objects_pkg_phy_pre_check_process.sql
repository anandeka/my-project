...
create or replace package body "PKG_PHY_PRE_CHECK_PROCESS" is
      ...
      if vn_pp_amt is null and vc_exch_rate_missing = 'N' then
        ...
        values
          (pc_corporate_id,
           ...
           cc.shipment_year || to_char(cc.payment_due_date, 'dd-Mon-yyyy'),
           null,
           pc_process,
           sysdate,
           pc_user_id,
  ...
  --------------------------------------------------------------------------------------------------------------------------
    ...
    --        Purpose                                   : Update Location Differential for Base Metals
    --
    --        Modification History
    --        Modified Date                             :
    --        Modified By                               :
  ...
  exception
    ...
    when others then
      vobj_error_log.extend;
      vobj_error_log(vn_eel_error_count) := pelerrorlogobj(pc_corporate_id,
                                                           'procedure sp_calc_m2m_quality_premimum',
                                                           'M2M-013',
                                                           'Code:' ||
                                                           sqlcode ||
                                                           'Message:' ||
  ...
  exception
    ...
    when others then
      vobj_error_log.extend;
      vobj_error_log(vn_eel_error_count) := pelerrorlogobj(pc_corporate_id,
                                                           'procedure sp_calc_m2m_product_premimum',
                                                           'M2M-013',
                                                           'Code:' ||
                                                           sqlcode ||
                                                           'Message:' ||
