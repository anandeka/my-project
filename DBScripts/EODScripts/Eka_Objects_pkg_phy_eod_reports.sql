...
vn_log_counter := vn_log_counter + 1;
  ...
  sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          vn_log_counter,
                          'gather_stats for purchase accrual starts');
...
insert into patd_pa_temp_data
  (internal_gmr_ref_no,
   ...
   is_afloat ,
    ...
    select gmr.internal_gmr_ref_no,
           ...
           gmr.corporate_id,
           null corporate_name,
           null conc_product_id,
           null conc_product_name,
           ...
           null payable_type,
           null cp_id,
           null counterparty_name,
           null pay_cur_id,
           null pay_cur_code,
           null pay_cur_decimal,
           grd.pcdi_id pcdi_id,
           spq.pledge_stock_id,
           grd.qty wet_qty,
  ...
  for cc in (select akc.corporate_name,
                    cm.cur_id,
                    cm.cur_code,
                    cm.decimals
               from ak_corporate       akc,
  ...
  loop
    update patd_pa_temp_data patd
       set patd.corporate_name   = cc.corporate_name,
           patd.base_cur_id      = cc.cur_id,
           patd.base_cur_code    = cc.cur_code,
           patd.base_cur_decimal = cc.decimals
  ...
  sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'update patd_pa_temp_data 1 corporate data');

  for cc in (select cpc.corporateid,
                    cpc.profit_center_id,
                    cpc.profit_center_name,
                    cpc.profit_center_short_name
  ...
  loop
    update patd_pa_temp_data patd
       set patd.profit_center_name       = cc.profit_center_name,
           patd.profit_center_short_name = cc.profit_center_short_name
     where patd.profit_center_id = cc.profit_center_id
  ...
  sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'update patd_pa_temp_data 2 profitcenter data');
  ...
  loop
    update patd_pa_temp_data patd
       set patd.cp_id             = cc.cp_id,
           patd.counterparty_name = cc.companyname,
           patd.conc_product_id   = cc.product_id,
           patd.conc_product_name = cc.product_desc,
           patd.pay_cur_id        = cc.invoice_currency_id
     where patd.pcdi_id = cc.pcdi_id
       and patd.corporate_id = pc_corporate_id;
  end loop;
     commit;
     ...
     sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                        vn_log_counter,
                        'update patd_pa_temp_data 4 cp name,product');
  for cc in (select cm.cur_id,
                    cm.cur_code,
                    cm.decimals
  ...
  loop
    update patd_pa_temp_data patd
       set patd.pay_cur_code    = cc.cur_code,
           patd.pay_cur_decimal = cc.decimals
     where patd.pay_cur_id = cc.cur_id
       and patd.corporate_id = pc_corporate_id;
  end loop;
  commit;
  ...
  sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'update patd_pa_temp_data 5 pay cur');
     vn_log_counter := vn_log_counter + 1;
     sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
                          pc_process_id,
...
insert into patd_pa_temp_data
  ...
  select grd.internal_gmr_ref_no,
   ...
   sp_calc_gmr_other_charges(pc_corporate_id,
                                    pd_trade_date,
                                    pc_process_id,
                                    'EOM')  ;
  vn_log_counter := vn_log_counter + 1;
        ...
        begin
         select cgcp.contract_price,
               ...
               where cgcp.internal_gmr_ref_no =
                 ...
                 and rownum < 2 ;-- As the data is at grd level
        exception
          when others then
            begin
...
pkg_metals_general.sp_get_gmr_tc_by_assay(cur_pur_accural_temp_rows.internal_gmr_ref_no,
                                                       ...
                                                       pc_dbd_id,
                                                       cur_pur_accural_temp_rows.gmr_price,
                                                       cur_pur_accural_temp_rows.gmr_price_unit_id,
                                                       cur_pur_accural_temp_rows.ash_id,
                                                       cur_pur_accural_temp_rows.dry_qty,
                                                       cur_pur_accural_temp_rows.grd_qty_unit_id,
...
pkg_metals_general.sp_get_gmr_rc_by_assay (cur_pur_accural_temp_rows.internal_gmr_ref_no,
                                                    ...
                                                    cur_pur_accural_temp_rows.payable_qty_unit_id,
...
(select tgoc.internal_gmr_ref_no,
 ...
 where tgoc.corporate_id = pc_corporate_id
) loop
update pa_purchase_accural_gmr pa_gmr
...
--- Base metal Not Event Based
  select gmr.corporate_id,
    ...
    from pcm_physical_contract_main pcm,
         ...
         gmr_goods_movement_record gmr,
         grd_goods_record_detail grd,
         (select vd.internal_gmr_ref_no,
                 max(vd.loading_date) loading_date
            from vd_voyage_detail vd
           where vd.process_id = pc_process_id
             and nvl(vd.status, 'Active') = 'Active'
           group by vd.internal_gmr_ref_no) vd,
         pcpd_pc_product_definition pcpd,
         pdm_productmaster pdm,
         qat_quality_attributes qat,
         phd_profileheaderdetails phd,
   ...
   where pcm.internal_contract_ref_no = pcdi.internal_contract_ref_no
     ...
     and 'TRUE' =  (case when trunc(gmr.eff_date, 'Mon') = trunc(pd_trade_date, 'Mon') and
  ...
  select corporate_id,
         ...
         case
           when invoice_invenotry_status = 'INVOICE' then
            case
           when dense_rank()
            over(partition by internal_grd_ref_no order by element_id) = 1 then
            invoice_invenotry_value
           else
            0
    ...
    from (select gmr.corporate_id,
           ...
           where pcm.internal_contract_ref_no =
             ...
             and gmr.is_deleted = 'N'
             and gmr.corporate_id = pc_corporate_id
             and cccp.price_unit_id = ppu_inven.internal_price_unit_id(+)
             and ppu_inven.price_unit_id = pum_inven.price_unit_id(+)
             and iid.new_invoice_price_unit_id =
                 ppu_invoice.internal_price_unit_id(+)
             and ppu_invoice.price_unit_id = pum_invoice.price_unit_id(+)
        ...
        from ( --concentrates event based
              select gmr.corporate_id,
               ...
               where pcm.internal_contract_ref_no =
                 ...
                 and gmr.is_deleted = 'N'
                 and gmr.corporate_id = pc_corporate_id
                 and cgcp.price_unit_id = ppu_inven.internal_price_unit_id(+)
                 and ppu_inven.price_unit_id = pum_inven.price_unit_id(+)
                 and iid.new_invoice_price_unit_id =
                     ppu_invoice.internal_price_unit_id(+)
                 and ppu_invoice.price_unit_id = pum_invoice.price_unit_id(+)
...
begin
  ...
  insert into temp_fcr
    ...
    select pc_corporate_id corporate_id,
           ...
           t.assay_qty,
           t.assay_qty_unit_id,
           qum_assay.qty_unit assay_qty_unit,
           t.payable_qty,
 ...
 where gmr.internal_gmr_ref_no = grd.internal_gmr_ref_no
   ...
   -- and asm.is_active ='Y'
     ...
     where t.internal_grd_ref_no = iam.internal_grd_ref_no(+)
       ...
       and t.payable_qty_unit_id = qum_payable.qty_unit_id
       and t.product_id = pdm.product_id
       and t.quality_id = qat.quality_id
...
commit;
  ...
  procedure sp_calc_customs_report(pc_corporate_id varchar2,
                                   ...
                                   pc_user_id      varchar2,
                                   pc_process      varchar2) is
  vn_qty_conversion number;
  vn_exch_rate             number;                            
  vn_price_conv             number;                            
  begin
     sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
                          pc_process_id,
...
delete from tcr_temp_cr where corporate_id = pc_corporate_id;
commit;
sp_gather_stats('tcr_temp_cr');
--
-- Base Metal Not Event Based 
--
insert into tcr_temp_cr
  (corporate_id,
   corporate_name,
   internal_contract_ref_no,
   contract_ref_no,
  ...
  select pcm.corporate_id,
         ...
         bccp.price_unit_id,
         ppu.product_price_unit_id,
         nvl(is1.invoice_currency_id, cm_base.cur_id) invoice_cur_id,
         nvl(cm_invoice.cur_code, cm_base.cur_code) invoice_cur_code,
         vd.loading_date,
         dense_rank() over(partition by grd.internal_gmr_ref_no order by rownum),
         'Base Metal Normal',
         1 grd_qty_conv_factor,
         1 cfx_inv_to_base,
         gmr.eff_date,
         cm_invtry.cur_id,
         cm_invtry.cur_code,
         1 cfx_loading_country_to_base,
         1 cfx_discharge_country_to_base,
         1 cp_to_prod_base_price_conv
    from gmr_goods_movement_record gmr,
         pcm_physical_contract_main pcm,
         pcdi_pc_delivery_item pcdi,
         pci_physical_contract_item pci,
   ...
   where gmr.internal_gmr_ref_no = grd.internal_gmr_ref_no
     ...
     and pcm.internal_contract_ref_no = pcdi.internal_contract_ref_no
     and grd.internal_gmr_ref_no = is1.internal_gmr_ref_no(+)
     and pcm.cp_id = phd.profileid(+)
     and grd.internal_contract_item_ref_no =
         pci.internal_contract_item_ref_no(+)
...
sp_gather_stats('tcr_temp_cr');
--
-- Convert Stock Qty into Product Base Qty
--
for cur_qty_update in
(
select t.grd_qty_unit_id,
       t.product_base_qty_unit_id
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.grd_qty_unit_id <> t.product_base_qty_unit_id
 group by t.grd_qty_unit_id,
          t.product_base_qty_unit_id
) loop
select pkg_general.f_get_converted_quantity(null,
                                            cur_qty_update.grd_qty_unit_id,
                                            cur_qty_update.product_base_qty_unit_id,
                                            1)
  into vn_qty_conversion
  from dual;
update tcr_temp_cr t
   set t.grd_qty_conv_factor = vn_qty_conversion
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.grd_qty_unit_id = cur_qty_update.grd_qty_unit_id
   and t.product_base_qty_unit_id = cur_qty_update.product_base_qty_unit_id;
end loop;
commit;  
     sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
...
--
-- Invoice Currency To Base CFX Rate
--
for cur_inv_base_rate in(
select t.invoice_cur_id,
       t.base_cur_id,
       bl_date
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.invoice_cur_id <> t.base_cur_id
 group by t.invoice_cur_id,
          t.base_cur_id,
          bl_date) loop
select pkg_general.f_get_converted_currency_amt(pc_corporate_id,
                                                cur_inv_base_rate.invoice_cur_id,
                                                cur_inv_base_rate.base_cur_id,
                                                cur_inv_base_rate.bl_date,
                                                1)
  into vn_exch_rate
  from dual;
update tcr_temp_cr t
   set t.cfx_inv_to_base = vn_exch_rate
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.invoice_cur_id = cur_inv_base_rate.invoice_cur_id
   and t.base_cur_id = cur_inv_base_rate.base_cur_id;
end loop;
commit;  
     sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
                          pc_process_id,
                          10004,
                          'Invoice Currency To Base CFX Rate Over');

--
-- Loading Country Currency To Base Currency CFX Rate
--
for cur_inv_base_rate in(
select t.loading_country_cur_id,
       t.base_cur_id,
       bl_date
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.loading_country_cur_id <> t.base_cur_id
 group by t.loading_country_cur_id,
          t.base_cur_id,
          bl_date) loop
select pkg_general.f_get_converted_currency_amt(pc_corporate_id,
                                                cur_inv_base_rate.base_cur_id,
                                                cur_inv_base_rate.loading_country_cur_id,
                                                cur_inv_base_rate.bl_date,
                                                1)
  into vn_exch_rate
  from dual;
update tcr_temp_cr t
   set t.cfx_base_to_loading_country = vn_exch_rate
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.loading_country_cur_id = cur_inv_base_rate.loading_country_cur_id
   and t.base_cur_id = cur_inv_base_rate.base_cur_id;
end loop;
commit;  
sp_gather_stats('tcr_temp_cr');
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10005,
                          'Loading Currency To Base Currency CFX Rate Over');
--
-- Discharge Country Currency To Base Currency CFX Rate
--
for cur_inv_base_rate in(
select t.discharge_country_cur_id,
       t.base_cur_id,
       bl_date
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.discharge_country_cur_id <> t.base_cur_id
 group by t.discharge_country_cur_id,
          t.base_cur_id,
          bl_date) loop
select pkg_general.f_get_converted_currency_amt(pc_corporate_id,
                                                cur_inv_base_rate.base_cur_id,
                                                cur_inv_base_rate.discharge_country_cur_id,
                                                cur_inv_base_rate.bl_date,
                                                1)
  into vn_exch_rate
  from dual;
update tcr_temp_cr t
   set t.cfx_base_to_discharge_country = vn_exch_rate
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.discharge_country_cur_id = cur_inv_base_rate.discharge_country_cur_id
   and t.base_cur_id = cur_inv_base_rate.base_cur_id;
end loop;
commit;
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10006,
                          'Discharge Currency To Base Currency CFX Rate Over');
--
-- Convert Price from Contract Price to Product Base Price Unit 
--
for cur_price_conv in (
select t.contract_price,
       t.price_unit_id,
       t.product_price_unit_id,
       t.bl_date
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.price_unit_id <> t.product_price_unit_id
   group by  t.contract_price,
             t.price_unit_id,
             t.product_price_unit_id,
             t.bl_date) loop
...
select pkg_phy_pre_check_process.f_get_converted_price(pc_corporate_id,
  ...
  from dual;
update tcr_temp_cr t
   set t.cp_to_prod_base_price_conv = vn_price_conv
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Normal'
   and t.price_unit_id = cur_price_conv.price_unit_id
   and t.product_price_unit_id = cur_price_conv.product_price_unit_id
   and t.bl_date = cur_price_conv.bl_date;
end loop;
COMMIT;
sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
...
insert into cr_customs_report
  ...
  select pc_process_id,
         ...
         loading_date,
         eff_date
    from (select internal_contract_ref_no,
                 contract_ref_no,
                 delivery_item_no,
                 corporate_id,
                 corporate_name,
                 ...
                 base_cur_id,
                 base_cur_code,
                 t.cfx_base_to_loading_country cfx_base_to_load_country,
                 t.cfx_base_to_discharge_country cfx_base_to_dis_country,
                 grd_qty qty,
                 grd_qty_unit_id qty_unit_id,
                 grd_qty_unit qty_unit,
                 invoice_date invoice_eff_date,
                 gmr_eff_date,
                 nvl(t.invoice_date, t.bl_date) final_invoice_date,
                 case
                   when t.internal_invoice_ref_no is not null and
                        t.record_rank = 1 then
                    t.invoice_item_amount
                   else
                    t.contract_price * t.cp_to_prod_base_price_conv *
                    t.grd_qty * grd_qty_conv_factor
                 end invoice_value,
                 invoice_cur_id,
                 invoice_cur_code,
                 inventory_cur_id,
                 inventory_cur_code,
                 loading_date,
                 gmr_eff_date eff_date
            from tcr_temp_cr t
           where t.corporate_id = pc_corporate_id
             and section_name = 'Base Metal Normal')
...
insert into tcr_temp_cr
  ...
  select pcm.corporate_id,
         ...
         ppu.product_price_unit_id,
         nvl(is1.invoice_currency_id, cm_base.cur_id) invoice_cur_id,
         nvl(cm_invoice.cur_code, cm_base.cur_code),
         vd.loading_date,
         dense_rank() over(partition by grd.internal_gmr_ref_no order by rownum) record_rank,
         'Base Metal Event',
         1 grd_qty_conv_factor,
         1 cfx_inv_to_base,
         gmr.eff_date,
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10009,
                          'Base Metal Event TCR Insert Over');       
...
select t.grd_qty_unit_id,
  ...
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Base Metal Event'
   and t.grd_qty_unit_id <> t.product_base_qty_unit_id
 group by t.grd_qty_unit_id,
          t.product_base_qty_unit_id) loop
select pkg_general.f_get_converted_quantity(null,
...
insert into cr_customs_report
  ...
  select pc_process_id,
    ...
    from (select internal_contract_ref_no,
            ...
            from tcr_temp_cr t
           where t.corporate_id = pc_corporate_id
             and section_name = 'Base Metal Event')
   group by internal_contract_ref_no,
            contract_ref_no,
            delivery_item_no,
            corporate_id,
...
insert into tcr_temp_cr
  ...
  select pcm.corporate_id,
         ...
         cm_countryd.cur_id,
         pdm.base_quantity_unit,
         ((grd.qty * nvl(asm.dry_wet_qty_ratio, 1)) / 100) grd_qty,
         grd.qty_unit_id,
         qum.qty_unit,
         is1.invoice_item_amount,
         cccp.contract_price,
         ...
         nvl(cm_invoice.cur_code, cm_base.cur_code) invoice_cur_code,
         vd.loading_date,
         dense_rank() over(partition by spq.internal_grd_ref_no order by spq.element_id) record_rank,
         'Concentrates Normal',
         1 grd_qty_conv_factor,
         1 cfx_inv_to_base,
         gmr.eff_date,
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10016,
                          'Concentrates Normal TCR Insertion Over');            
...
select t.grd_qty_unit_id,
  ...
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Concentrates Normal'
   and t.grd_qty_unit_id <> t.product_base_qty_unit_id
 group by t.grd_qty_unit_id,
          t.product_base_qty_unit_id
) loop
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10021,
                          'Price Conversion Over For Concentrates Normal');   
--
-- Payable Qty Convert to Underlying Product Base Unit
--
for cur_qty_update in
(
select t.payable_qty_unit_id,
       t.underlying_base_quantity_unit
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Concentrates Normal'
   and t.payable_qty_unit_id <> t.underlying_base_quantity_unit
 group by t.payable_qty_unit_id,
       t.underlying_base_quantity_unit
) loop
select pkg_general.f_get_converted_quantity(null,
                                            cur_qty_update.payable_qty_unit_id,
                                            cur_qty_update.underlying_base_quantity_unit,
                                            1)
  into vn_qty_conversion
  from dual;
update tcr_temp_cr t
   set t.payable_qty_conv_factor = vn_qty_conversion
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Concentrates Normal'
   and t.payable_qty_unit_id = cur_qty_update.payable_qty_unit_id
   and t.underlying_base_quantity_unit = cur_qty_update.underlying_base_quantity_unit;
end loop;
commit;
sp_gather_stats('tcr_temp_cr');
sp_gather_stats('cr_customs_report');
sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
                          pc_process_id,
                          10022,
...
insert into cr_customs_report
  ...
  select pc_process_id,
    ...
    from (select internal_contract_ref_no,
                 ...
                 t.cfx_base_to_discharge_country cfx_base_to_dis_country,
                 case
                   when t.record_rank = 1 then
                    t.grd_qty * t.grd_qty_conv_factor
                   else
                    0
                 end qty,
                 grd_qty_unit_id,
                 grd_qty_unit,
                 t.invoice_date invoice_eff_date,
                 gmr_eff_date gmr_eff_date,
                 nvl(invoice_date, bl_date) final_invoice_date,
                 case
                   when internal_invoice_ref_no is not null and
                        t.gmr_rank = 1 then
                    t.invoice_item_amount
                   when internal_invoice_ref_no is not null and
                        t.gmr_rank <> 1 then
                    ...
                    t.contract_price * t.cp_to_prod_base_price_conv *
                    t.payable_qty * t.payable_qty_conv_factor
                 end invoice_value,
                 invoice_cur_id,
                 invoice_cur_code,
                 inventory_cur_id,
...
insert into tcr_temp_cr
  ...
  select pcm.corporate_id,
         ...
         cm_countryd.cur_id,
         pdm.base_quantity_unit,
         ((grd.qty * nvl(asm.dry_wet_qty_ratio, 1)) / 100),
         grd.qty_unit_id,
         qum.qty_unit,
         is1.invoice_item_amount,
         cgcp.contract_price,
         ...
         nvl(cm_invoice.cur_code, cm_base.cur_code) invoice_cur_code,
         vd.loading_date,
         dense_rank() over(partition by spq.internal_grd_ref_no order by spq.element_id) ,
         'Concentrates Event',
         1 grd_qty_conv_factor,
         1 cfx_inv_to_base,
         ...
         cm_invtry.cur_id inventory_cur_id,
         cm_invtry.cur_code inventory_cur_code,
         1 cfx_base_to_loading_country,
         1 cfx_base_to_discharge_country,
         1 cp_to_prod_base_price_conv,
         spq.payable_qty,
         spq.qty_unit_id,
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10024,
                          'Concentrates Event TCR Insertion Over');            
...
select t.grd_qty_unit_id,
  ...
  from tcr_temp_cr t
 where t.corporate_id = pc_corporate_id
   and t.section_name = 'Concentrates Event'
   and t.grd_qty_unit_id <> t.product_base_qty_unit_id
 group by t.grd_qty_unit_id,
          t.product_base_qty_unit_id
) loop
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10029,
                          'Price Conversion Over For Concentrates Event');   
--
-- Payable Qty Convert to Underlying Product Base Unit
--
for cur_qty_update in
...
sp_eodeom_process_log(pc_corporate_id,
                          ...
                          pc_process_id,
                          10031,
                          'Concentrates Event End');   
 
sp_eodeom_process_log(pc_corporate_id,
                          pd_trade_date,
                          pc_process_id,
                          10032,
                          'End of Customs Report');                  
...
select gmr_ref_no,
          ...
          from gmr_goods_movement_record gmr,
               ...
               (select spq.internal_gmr_ref_no,
                 ...
                 group by spq.internal_gmr_ref_no,
                          spq.internal_grd_ref_no,
                          spq.assay_header_id,
                          spq.weg_avg_pricing_assay_id) spq,
               ash_assay_header ash,
               asm_assay_sublot_mapping asm,
               aml_attribute_master_list aml,
               qum_quantity_unit_master qum_grd,
  ...
  vn_line_no:=1;
    ...
    loop  
      ...
      insert into fce_feed_consumption_element
        (process_id,
         ...
         asaay_qty_unit,
         payable_qty,
         original_payable_qty,
         payable_qty_unit_id,
         payable_qty_unit,
         original_asaay_qty_unit_id,        
         original_payable_qty_unit_id,        
         underlying_product_id,
      ...
      values
        (pc_process_id,
         ...
         cur_feed_rows.payable_qty, -- original for rc calculation
         cur_feed_rows.base_quantity_unit_id,
         cur_feed_rows.base_quantity_unit,
         cur_feed_rows.assay_qty_unit_id,-- Original for conversion
         cur_feed_rows.payable_qty_unit_id,-- Original for conversion
    ...
    commit;
--    
-- Convery Wet and Dry Qty to Product Base Unit
--
vn_line_no:=2;
for cur_wet_dry in( 
select fc.grd_qty_unit_id,
       fc.conc_base_qty_unit_id,
       ucm.multiplication_factor
  from fc_feed_consumption        fc,
       ucm_unit_conversion_master ucm
 where fc.process_id = pc_process_id
   and fc.grd_qty_unit_id <> fc.conc_base_qty_unit_id
   and ucm.from_qty_unit_id = fc.grd_qty_unit_id
   and ucm.to_qty_unit_id = grd_qty_unit_id
 group by fc.grd_qty_unit_id,
          fc.conc_base_qty_unit_id,
          ucm.multiplication_factor)loop
Update fc_feed_consumption        fc
set fc.grd_wet_qty = fc.grd_wet_qty * cur_wet_dry.multiplication_factor,
fc.grd_dry_qty = fc.grd_dry_qty * cur_wet_dry.multiplication_factor
where fc.process_id = pc_process_id
and fc.grd_qty_unit_id = cur_wet_dry.grd_qty_unit_id
and fc.conc_base_qty_unit_id= cur_wet_dry.conc_base_qty_unit_id;
end loop;
commit;
vn_line_no:=3;
--
-- Convert Payable and Assay Qty To Base Unit
--
for cur_qty_conv in(       
select fce.original_asaay_qty_unit_id,
       fce.underlying_base_qty_unit_id,
       ucm.multiplication_factor
  from fce_feed_consumption_element fce,
       ucm_unit_conversion_master   ucm
 where fce.process_id = pc_process_id
   and fce.original_asaay_qty_unit_id <> fce.underlying_base_qty_unit_id
   and ucm.from_qty_unit_id = fce.original_asaay_qty_unit_id
   and ucm.to_qty_unit_id = fce.underlying_base_qty_unit_id
   and fce.payable_qty_unit_id is not null -- not needed for penalty
   group by fce.original_asaay_qty_unit_id,
       fce.underlying_base_qty_unit_id,
       ucm.multiplication_factor) loop
       update fce_feed_consumption_element fce
       set fce.assay_qty = fce.assay_qty * cur_qty_conv.multiplication_factor,
       fce.payable_qty = fce.payable_qty * cur_qty_conv.multiplication_factor
       where fce.process_id = pc_process_id
       and fce.original_asaay_qty_unit_id = cur_qty_conv.original_asaay_qty_unit_id
       and fce.underlying_base_qty_unit_id = cur_qty_conv.underlying_base_qty_unit_id;
end loop;       
commit;    
vn_line_no:=4;
--    
-- Update TC / RC and Penalty 
--
-- 
-- 1 TC : This calculation has to consider dry qty of concentrates
--      : Qty has to be converted from Stock Qty Unit to TC Weight Unit 
--      : And Multiplied by TC Value 
--
for cur_tc in (
select fc.internal_gmr_ref_no,
  ...
  from getc_gmr_element_tc_charges t,
       fc_feed_consumption         fc,
       ucm_unit_conversion_master  ucm
 where fc.parent_internal_gmr_ref_no = t.internal_gmr_ref_no
 AND fc.parent_internal_grd_ref_no = t.internal_grd_ref_no
   and ucm.from_qty_unit_id = fc.original_grd_qty_unit_id
   and ucm.to_qty_unit_id = t.tc_weight_unit_id
   and fc.process_id = pc_process_id
   and t.process_id = pc_process_id
   and nvl(t.tc_value,0) <> 0
) loop
Update fce_feed_consumption_element fce
set fce.tc_amount = cur_tc.converted_qty * cur_tc.tc_value 
where fce.internal_grd_ref_no = cur_tc.internal_grd_ref_no
and fce.internal_gmr_ref_no = cur_tc.internal_gmr_ref_no
and fce.element_id = cur_tc.element_id
and fce.process_id = pc_process_id;
end loop;
commit;
vn_line_no:=5;
--
-- 2. RC : RC is on payable qty 
-- 
for cur_rc in (
select fce.internal_gmr_ref_no,
       fce.element_id,
       fce.original_payable_qty * ucm.multiplication_factor * gerc.rc_value rc_value,
       fce.internal_grd_ref_no
  from gerc_gmr_element_rc_charges  gerc,
       fce_feed_consumption_element fce,
       ucm_unit_conversion_master   ucm
 where gerc.process_id = pc_process_id
   and gerc.process_id = fce.process_id
   and gerc.internal_gmr_ref_no = fce.parent_internal_gmr_ref_no
  and gerc.internal_grd_ref_no = fce.parent_internal_grd_ref_no
   and gerc.element_id = fce.element_id
   and ucm.from_qty_unit_id = fce.original_payable_qty_unit_id
   and ucm.to_qty_unit_id = gerc.rc_weight_unit_id
   and nvl(gerc.rc_value, 0) <> 0) loop
Update fce_feed_consumption_element fce
set fce.rc_amount = cur_rc.rc_value
where fce.process_id = pc_process_id
and fce.internal_gmr_ref_no = cur_rc.internal_gmr_ref_no
and fce.internal_grd_ref_no = cur_rc.internal_grd_ref_no
and fce.element_id = cur_rc.element_id;
end loop;   
commit;
vn_line_no:=6;
--
...
select fc.internal_gmr_ref_no,
 ...
 and fc.parent_internal_grd_ref_no = t.internal_grd_ref_no
   and ucm.from_qty_unit_id = fc.original_grd_qty_unit_id
   and ucm.to_qty_unit_id = t.pc_weight_unit_id
   and fc.process_id = pc_process_id
   and t.process_id = pc_process_id
   and nvl(t.pc_value,0) <> 0
) loop
Update fce_feed_consumption_element fce
set fce.penality_amount = cur_pc.converted_qty * cur_pc.pc_value 
where fce.internal_grd_ref_no = cur_pc.internal_grd_ref_no
and fce.internal_gmr_ref_no = cur_pc.internal_gmr_ref_no
and fce.element_id = cur_pc.element_id
and fce.process_id = pc_process_id;
end loop;
commit;
  exception
  ...
  loop
    ...
    if cur_closing_rows.qty_type in ('Payable', 'Returnable') then
      ...
      if vc_gmr_treatment_cur_id is not null and
         ...
         vc_gmr_treatment_cur_id <> cur_closing_rows.pay_cur_id then
      
        vn_base_gmr_treatment_charge := round(pkg_general.f_get_converted_currency_amt(cur_closing_rows.corporate_id,
                                                                                       vc_gmr_treatment_cur_id,
        ...
        vn_base_gmr_treatment_charge := vn_gmr_treatment_charge;
      end if;
      if vc_gmr_refine_cur_id is not null and
         cur_closing_rows.pay_cur_id is not null and
         vc_gmr_refine_cur_id <> cur_closing_rows.pay_cur_id then
      
        vn_base_gmr_refine_charge := round(pkg_general.f_get_converted_currency_amt(cur_closing_rows.corporate_id,
                                                                                    vc_gmr_refine_cur_id,
                                                                                    cur_closing_rows.pay_cur_id,
                                                                                    pd_trade_date,
                                                                                    vn_gmr_refine_charge),
      ...
      if vc_gmr_penality_cur_id is not null and
         cur_closing_rows.pay_cur_id is not null and
         vc_gmr_penality_cur_id <> cur_closing_rows.pay_cur_id then
      
        vn_base_gmr_penality_charge := round(pkg_general.f_get_converted_currency_amt(cur_closing_rows.corporate_id,
                                                                                      vc_gmr_penality_cur_id,
                                                                                      cur_closing_rows.pay_cur_id,
                                                                                      pd_trade_date,
                                                                                      vn_gmr_penality_charge),
        ...
        vn_base_gmr_penality_charge := vn_gmr_penality_charge;
      end if;
...
begin
  for cc in (select grd.internal_gmr_ref_no internal_gmr_ref_no,
              ...
              where ash.ash_id = asm.ash_id
                ...
                and spq.element_id = aml.attribute_id
                and exists
              (select * from  pcth_pc_treatment_header       pcth,
             ...
             select dgrd.internal_gmr_ref_no,
              ...
              where ash.ash_id = asm.ash_id
                ...
                and spq.is_active = 'Y'
                and exists (select * from  pcth_pc_treatment_header       pcth,
                                       ted_treatment_element_details  red,
                                       pcetc_pc_elem_treatment_charge pcetc,
                                       tqd_treatment_quality_details  tqd,
                                       gth_gmr_treatment_header       gth
      ...
      begin
         select cgcp.contract_price,
             ...
             where cgcp.internal_gmr_ref_no = cc.internal_gmr_ref_no
               ...
               and rownum < 2; -- B'COS DATA IS AT GRD LEVEL
      exception
        when others then
          begin
          select cccp.contract_price,
...
begin
  ...
  for cc in (select gmr.internal_gmr_ref_no,
              ...
              where gmr.internal_gmr_ref_no = grd.internal_gmr_ref_no
                ...
                and grd.internal_contract_item_ref_no =
                    pci.internal_contract_item_ref_no
                 --   AND GRD.INTERNAL_GRD_REF_NO='GRD-8898'
               and exists
                    (select *
    ...
    begin
      select pcepc.include_ref_charges
       ...
       where pcpch.pcpch_id = pcepc.pcpch_id
         ...
         and pcepc.pcpch_id = cc.payable_rule_id
  ...
  for cc in (select gmr.internal_gmr_ref_no,
              ...
              where gmr.internal_gmr_ref_no = grd.internal_gmr_ref_no
                ...
                and grd.internal_contract_item_ref_no =
                    pci.internal_contract_item_ref_no
                and exists (
                select * from pcaph_pc_attr_penalty_header  pcaph,
...
procedure sp_calc_gmr_other_charges(pc_corporate_id varchar2,
  ...
  cursor cur_each_gmr(pc_internal_gmr_ref_no varchar2) is
    select tgoc.internal_gmr_ref_no,
           ...
           tgoc.dry_qty,
           tgoc.wet_qty,
           tgoc.is_apply_container_charge,
           tgoc.is_apply_freight_allowance,
           tgoc.is_invoiced,
           tgoc.latest_internal_invoice_ref_no,
           pcmac.*
      from pcmac_pcm_addn_charges     pcmac,
           tgoc_temp_gmr_other_charge tgoc
     where tgoc.internal_contract_ref_no = pcmac.int_contract_ref_no
       and tgoc.internal_gmr_ref_no = pc_internal_gmr_ref_no
       and tgoc.corporate_id = pc_corporate_id
       and pcmac.corporate_id = pc_corporate_id
       and pcmac.is_active = 'Y'
     order by tgoc.internal_gmr_ref_no,
              pcmac.addn_charge_id;
begin
  delete from tgoc_temp_gmr_other_charge t
   where t.corporate_id = pc_corporate_id;
  commit;
  insert into tgoc_temp_gmr_other_charge
    (corporate_id,
     internal_gmr_ref_no,
     internal_contract_ref_no,
     is_wns_created,
     ...
     is_apply_freight_allowance,
     latest_internal_invoice_ref_no)
    select patd.corporate_id,
           patd.internal_gmr_ref_no,
           patd.internal_contract_ref_no,
           patd.is_wns_created,
           patd.is_invoiced,
           sum(nvl(patd.no_of_bags, 0)) no_of_bags,
           0 total_sublots,
           sum(patd.dry_qty) dry_qty,
           sum(patd.wet_qty) wet_qty,
           0,
           0,
           0,
           0,
     ...
     where patd.corporate_id = pc_corporate_id
       ...
       and patd.is_pledge = 'N'
       and patd.payable_type in ('Payable', 'Returnable') -- We need non-pledge, non penalty section to calcualte other charges
     group by patd.corporate_id,
              patd.internal_gmr_ref_no,
              patd.internal_contract_ref_no,
              patd.is_wns_created,
              patd.is_invoiced,
  ...
  sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'Insert into tgoc_temp_gmr_other_charge over');
  commit;
  --
  -- Update Total No of Slots Per GMR
  --
   update tgoc_temp_gmr_other_charge t
      set t.no_of_sublots = (select sum(ash.no_of_sublots) total_sub_lots
                               from ash_assay_header ash
                              where ash.assay_type =
                                    'Weighing and Sampling Assay'
                                and ash.is_active = 'Y'
                                and ash.internal_gmr_ref_no =
                                    t.internal_gmr_ref_no
...
sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'Update Total No of Sub Lots Per GMR over');  
  ...
  loop
    ...
    loop
      ...
      -- If Rate then multiply by total no of bub lots for the GMR, if Flat take the value as is
      --
      ...
      --
      -- Handling Charges, If Rate then multiply by total no of bub lots for the GMR, If Flat take the value as is
      --
      ...
      --
      -- Location Value , If Rate then multiply by total no of bub lots for the GMR, If Flat take the value as is
      --
      ...
      --
      -- Freight Allowance , If Rate then multiply by total no of bub lots for the GMR, If Flat take the value as is
      --
      -- also ask how to get this is applicable for gmr or not
        ...
        if cur_each_gmr_rows.is_invoiced ='Y' then
            select count(*)
               ...
               where ioc.internal_invoice_ref_no =
                 ...
                 and ioc.other_charge_cost_id = 'CC-1';-- Hard code value for Container Charges
          end if;
              ...
              begin
                select count(distinct grd.container_no)
                 ...
                 where grd.status = 'Active'
                   and grd.is_deleted = 'N'
                   and grd.container_no is not null
                   and grd.container_size is not null
                   and grd.internal_gmr_ref_no = cur_each_gmr_rows.internal_gmr_ref_no
    ...
    update tgoc_temp_gmr_other_charge tgoc
       set tgoc.small_lot_charge  = vn_small_lot_charge,
           tgoc.container_charge  = vn_container_charge,
           tgoc.sampling_charge   = vn_sampling_charge,
           tgoc.handling_charge   = vn_handling_charges,
           tgoc.location_value    = vn_location_value,
           tgoc.freight_allowance = vn_freight_allowance
     where tgoc.corporate_id = pc_corporate_id
       and tgoc.internal_gmr_ref_no = cur_all_gmr_rows.internal_gmr_ref_no;
  end loop;
  commit;
  vn_log_counter := vn_log_counter + 1;
  sp_eodeom_process_log(pc_corporate_id,
                        ...
                        pc_process_id,
                        vn_log_counter,
                        'TGOC_TEMP_GMR_OTHER_CHARGE Population Over');  
end;
end; 
/
