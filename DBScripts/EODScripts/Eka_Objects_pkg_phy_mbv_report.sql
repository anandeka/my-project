 ...
 insert into pfrd_price_fix_report_detail
   ...
   select pc_process_id process_id,
    ...
    where fmuh.fmuh_id = fmed.fmuh_id
      ...
      and ucm.to_qty_unit_id = pdm.base_quantity_unit
      and ucm.is_active = 'Y'
      and ppu.product_price_unit_id = fmed.price_unit_id
      and ppu.cur_id = cm.cur_id
  ...
  begin
    ...
    for cur_der in (select nvl(sum(case
                                 when dpd.pnl_type = 'Unrealized' then
                                  ...
                                  0
                               end),0) der_unrealized_pnl,
                           nvl(sum(case
                                 when dpd.pnl_type = 'Realized' then
                                  dpd.pnl_in_base_cur
                                 else
                                  0
                               end),0)  der_realized_pnl,
                           nvl(sum(case
                                 when dpd.pnl_type = 'Realized' then
                                  dpd.closed_quantity
                                 else
                                  0
                               end),0)  der_realized_qty,
                           dpd.instrument_id,
                           dpd.derivative_prodct_id
                      from dpd_derivative_pnl_daily dpd
                     where dpd.process_id = pc_process_id
    ...
    for cur_inv_section in (select css.product_id,
                                   nvl(sum(css.priced_arrived_qty +
                                       ...
                                       css.unpriced_delivered_qty),0) total_inv_qty,
                                   nvl(sum(css.priced_arrived_qty -
                                       css.priced_delivered_qty),0) priced_inv_qty,
                                   nvl(sum(css.unpriced_arrived_qty -
                                       css.unpriced_delivered_qty),0) unpriced_inv_qty
                              from css_contract_status_summary css
                             where css.process_id = pc_process_id
                             group by css.product_id)
    loop
...
commit;
    ...
    --
    update mbv_metal_balance_valuation mbv
       set mbv.hedge_effectiveness = nvl((case when mbv.qty_to_be_hedged <> 0 then 1 - 
        (mbv.qty_to_be_hedged + mbv.actual_hedged_qty) / mbv.qty_to_be_hedged 
        else 0 end),0) * 100
     where mbv.process_id = pc_process_id;
    commit;
    ...
    for cur_der_ref_price_diff in (select mbvd.product_id,
                                          nvl(sum(mbvd.value_diff_in_base_ccy),0) der_ref_price_diff_value
    ...
    for cur_phy_ref_price_diff in (select mbvp.product_id,
                                          nvl(sum(mbvp.referential_price_in_base_cur *
                                              mbvp.qty),0) phy_ref_price_diff_value
    ...
    --
    for cur_contango_dueto_qty in (select mbva.product_id,
                                          nvl(sum(mbva.contango_due_to_qty_and_price),0) contango_dueto_qty_price
  ...
  update mbv_metal_balance_valuation mbv
     set mbv.phy_unr_price_inv_price = (case when mbv.priced_inv_qty < 0 then cur_pfhr1.wap_sales_price_fixations 
                                         else cur_pfhr1.wap_purchase_price_fixations end),
...
--
update mbv_metal_balance_valuation mbv
set mbv.unr_phy_priced_inv_pnl = ( mbv.month_end_price - phy_unr_price_inv_price) * mbv.priced_inv_qty
   where mbv.process_id = pc_process_id;
   commit;
--
...
select mbvp.product_id,
       nvl(sum(case
             when mbvp.purchase_sales = 'P' then
              ...
              0
           end),0) unr_phy_priced_na_pnl,
       nvl(sum(case
             when mbvp.purchase_sales = 'S' then
              mbvp.unrealized_pnl_in_base_cur
             else
              0
           end),0) unr_phy_priced_nd_pnl
  from mbv_phy_postion_diff_report mbvp
 where mbvp.process_id = pc_process_id
...
commit;
  ...
  vc_error_msg := 'Qty Total P and L Till Date';
    ...
    -- 
    update mbv_metal_balance_valuation mbv
       set mbv.phy_realized_cb_pnl = mbv.phy_realized_ob_pnl + mbv.phy_realized_pnl
     where mbv.process_id = pc_process_id;
    commit;
...
--    
for cur_md_debt in(
select md.product_id, md.debt_qty from md_metal_debt md
where md.process_id = pc_process_id) loop
update mbv_metal_balance_valuation mbv
...
exception
  ...
  vc_error_msg :='Start of Allocation Report';
    ...
    select pc_process_id process_id,
       ...
       fmpfh.priced_qty * ucm.multiplication_factor purchase_qty,
       0 sales_qty,
       fmed.qty_unit_id,
       qum.qty_unit,
       fmed.provisional_price price,
 ...
 where fmuh.fmuh_id = fmed.fmuh_id
   ...
   and axs.eff_date >= vd_prev_eom_date
   and axs.eff_date<=pd_trade_date
   and fmed.price_unit_id = ppu.product_price_unit_id
   and ppu.cur_id = cm.cur_id
   and ppu.weight_unit_id = qum_ppu.qty_unit_id
   and ucm.from_qty_unit_id = fmed.qty_unit_id
   and ucm.to_qty_unit_id = pdm.base_quantity_unit
    ...
    insert into mbv_allocation_report_header
      ...
      select process_id,
        ...
        from (select pc_process_id process_id,
                     ...
                     mbv.product_id,
                     mbv.product_desc,
                     sum( case when mbv.eod_trade_date < pd_trade_date then mbv.purchase_qty else 0 end ) -
                     sum(case when mbv.eod_trade_date < pd_trade_date then mbv.sales_qty else 0 end) opening_balance,
                     sum(case
