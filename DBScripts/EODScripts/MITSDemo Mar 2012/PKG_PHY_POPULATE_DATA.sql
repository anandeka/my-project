...
set pci.price_option_call_off_status =
(select pcdi.price_option_call_off_status from pcdi_pc_delivery_item pcdi
where pcdi.dbd_id =gvc_dbd_id
and pcdi.pcdi_id = pci.pcdi_id
and pci.dbd_id = gvc_dbd_id)
where  pci.dbd_id = gvc_dbd_id;
-- Called Off  Non Event Based
update pci_physical_contract_item pci
   set (pci.price_basis, pci.qp_start_date, pci.qp_end_date, pci.qp_period_type, pci.pcbpd_id) = --
        (select pcbpd.price_basis,
                pofh.qp_start_date,
                pofh.qp_end_date,
                pocd.qp_period_type,
                pcbpd.pcbpd_id
          ...
          where poch.poch_id = pocd.poch_id
            ...
            and pcbpd.dbd_id = gvc_dbd_id
            and pcbph.dbd_id = gvc_dbd_id
 ...
 where pci.dbd_id = gvc_dbd_id
   and pci.price_option_call_off_status in
       ('Called Off', 'Not Applicable');
--               
-- Not Called Off (Need to check O/J required ??
-- 
update pci_physical_contract_item pci
   set (pci.price_basis, pci.qp_start_date, pci.qp_end_date, pci.qp_period_type, pci.pcbpd_id) = --
(select pcbpd.price_basis,
  ...
  from pci_physical_contract_item    pci,
       ...
       where t.dbd_id = gvc_dbd_id
       and     t.is_active = 'Y') ppfh,
       (select * from 
       pfqpp_phy_formula_qp_pricing  t
       where t.dbd_id = gvc_dbd_id
       and   t.is_active = 'Y') pfqpp
 where pci.internal_contract_item_ref_no =
   ...
   and pci.price_option_call_off_status = 'Not Called Off'
   and pcbpd.element_id is null)
 where pci.dbd_id = gvc_dbd_id
   and pci.price_option_call_off_status in ('Not Called Off');
 -- Called Off/Not Called Off  Event Based
 update pci_physical_contract_item pci
 set(pci.qp_start_date, pci.qp_end_date) =
       (select dieqp.expected_qp_start_date,
               dieqp.expected_qp_end_date
          from di_del_item_exp_qp_details dieqp
         where dieqp.pcdi_id = pci.pcdi_id
           and dieqp.pcbpd_id = pci.pcbpd_id
    ...
    insert into agh_alloc_group_header
      ...
      select decode(int_alloc_group_id,
             ...
             to_timestamp(decode(realized_creation_date,
                                 ...
                                 realized_creation_date),
                          'yyyy-mm-dd hh24:mi:ss.FF'),
             --decode(today_status, 'Empty_String', null, today_status),
             decode(internal_action_ref_no,
                    'Empty_String',
                    null,
                    internal_action_ref_no),
